...
typedef Zeni_Collision::Ray Collider_Type;
...
: m_time(Timer_HQ::get_reference().get_time()),
m_angle(0.0f),
...
font(&Fonts::get_reference().get_font("font")),
game(&Game::get_reference()),
font_color(1.0f, 0.5f, 1.0f, 0.5f)
{
...
void MyCustomGamestate::perform_logic() {
  Time_HQ new_time = Timer_HQ::get_reference().get_time();
  const float time_diff = float(new_time.get_seconds_since(m_time));
  m_time = new_time;
...
void MyCustomGamestate::render() {
  ...
  font->render_text(itoa(game->get_fps()), 2, 2, font_color);
}
